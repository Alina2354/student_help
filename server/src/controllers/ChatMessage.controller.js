...
class ChatMessageController {
  ...
  static async getMessagesByUserId(req, res) {
    try {
      ...
      res.status(200).json(formatResponse(200, "Сообщения пользователя", messages));
    } catch (error) {
      res.status(500).json(formatResponse(500, "Ошибка сервера", null, error.message));
    }
  }
  ...
  static async getMessageById(req, res) {
    try {
      ...
      if (!message) {
        return res.status(404).json(formatResponse(404, "Сообщение не найдено"));
      }
  ...
  static async createMessage(req, res) {
    try {
      ...
      if (!content || !sender) {
        return res.status(400).json(formatResponse(400, "Контент и отправитель обязательны"));
      }

      if (!["user", "assistant"].includes(sender)) {
        return res.status(400).json(formatResponse(400, "Отправитель должен быть 'user' или 'assistant'"));
      }

      // Если пользователь не авторизован, не сохраняем сообщение
      if (!userId) {
        const message = await chatMessageService.createMessageForGuest(content, sender);
        return res.status(200).json(formatResponse(200, "Сообщение получено (не сохранено)", message));
      }

      // Если пользователь авторизован, сохраняем сообщение
      ...
      res.status(500).json(formatResponse(500, "Ошибка при создании сообщения", null, error.message));
    }
  }
  ...
  static async updateMessage(req, res) {
    try {
      ...
      res.status(500).json(formatResponse(500, "Ошибка при обновлении сообщения", null, error.message));
    }
  }
  ...
  static async deleteMessage(req, res) {
    try {
      ...
      res.status(500).json(formatResponse(500, "Ошибка при удалении сообщения", null, error.message));
    }
  }
}
